...
//	protected static $IDField = "userID"; // *** default assumes table tbUsers
	...
	protected function attributes() { // ?static? (trying not to use $obj param) 
	...
	private function hasAttribute($attribute) { // called by: instantiate  ***DONT KNOW WHY I HAVE TO PASS IN $obj
		...
		echo "in hasAttribute for Attribute: " . $attribute . " <br> Search list: " . $listObjVars . "<br/>";
		
		// We don't care about the value, we just want to know if key exits
		return array_key_exists($attribute, $objectVars);
	}

	protected function sanitizedAttributes() {  //   ***DONT KNOW WHY I HAVE TO PASS IN $obj
		global $database;
	...
	private static function instantiate($record) {
		...
		if (is_array($record)) {
			...
			foreach($record as $attribute=>$value){
				...
				// **** DEBUG 3/24/15 // echo "in INSTANTIATE : " . $attribute . " - value: " . $value . "<br/>". $dbgHsAttr; 
